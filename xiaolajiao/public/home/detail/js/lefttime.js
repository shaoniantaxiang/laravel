...
//      Temp = minuteZero + minuteLeft + _minute + secondZero + secondLeft + _second;
    }
    else
    {
      if (minuteLeft > 0)
  ...
  if (auctionDate <= 0 || Temp == '')
  {
    ...
    stopclock();
	if(($('#leftTime').attr('data-nowtime') < $('#leftTime').attr('data-endtime')) && (($('#addReservation').attr('data-res') == 1 || $('#addReservation').attr('data-res') == 3) || ($('#addCarttmp').attr('data-res') == 1 || $('#addCarttmp').attr('data-res') == 3))) {
...
function onload_leftTime(now_time)
{
  ...
  try
  {
    _GMTEndTime = $('#leftTime').attr('data-endtime');
    // 剩余时间
    _day = day;
    _hour = hour;
    _minute = minute;
  ...
  if (_GMTEndTime > 0)
  {
    if ($('#leftTime').attr('data-nowtime') == undefined)
    {
//      var tmp_val = parseInt(_GMTEndTime) - parseInt(cur_date.getTime() / 1000 + cur_date.getTimezoneOffset() * 60);
    ...
    {
      var tmp_val = parseInt(_GMTEndTime) - $('#leftTime').attr('data-nowtime');
    }
    if (tmp_val > 0)
    {
      auctionDate = tmp_val;
